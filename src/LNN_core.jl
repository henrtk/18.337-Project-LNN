...
"""
struct LagrangiaNN{M, R, P}
    model::M
    re::R
    p::P
end 

    function LagrangiaNN(model; p = nothing)
        _p, re = Flux.destructure(model)
        p = p === nothing ? _p : p
        return new{typeof(model), typeof(re), typeof(p)}(model, re, p)
    end
    
Flux.trainable(lnn::LagrangiaNN) = (lnn.p,)
...
function _lagrangian_forward(re, p, q_qdot)
    ...
    ∇_q∇ₚᵀL = Zygote.jacobian(q_qdot-> Zygote.jacobian(L,q_qdot)[N+1:end], q_qdot)[1]
    ∇_qL = Zygote.jacobian(L, q_qdot)[1][1:N]
    
    # Pseudo-inverse to avoid singularities
    ∇²L⁻¹ = pinv(Zygote.hessian(L, qp)[1])

    qdotdot = ∇²L⁻¹*(∇_qL - ∇_q∇ₚᵀL * qdot)
    return [qdot, qdotdot]
end

(Lnn::LagrangianNN)(q_qdot, p = lnn.p) = _lagrangian_forward(hnn.re, p, q_qdot)
...
struct NeuralLagrangian{M,P,RE,T,A,K} <: NeuralDELayer
    ...
    function NeuralLagrangian(model, tspan; p = nothing, args...; kwargs...)
        ...
        new{    
            ...
            typeof(args), 
            typeof(kwargs)
            }(lnn, lnn.p, re, tspan, args, kwargs)
...
# Define the output of the LNN
function (lnnL::NeuralLagrangian)(q_qdot, p = lnn.p)
    ...
    sensitivity = InterpolatingAdjoint(autojacvec = false)
